...
async def historical_blocks_txs_sync():
    ...
    table: lancedb.table = db.open_table("transactions")

    to_block: int = table.to_polars().select('block_number').sort(
        by='block_number', descending=False).collect()['block_number'][0] - 1

    from_block = to_block - (7_200 * 14)
    db_batch_size: int = 2_500  # number of blocks to process per batch
